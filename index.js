...
function firstNamesAllCaps(runner) {
  let firstName= [];
  runner.map(function(item){
    return firstName.push(item.first_name.toUpperCase());
  })
  return firstName;
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  let size = 
  runners.filter(item => item.shirt_size === tShirtSize);
  return size;
}
...
function tallyUpDonations(runners) {
 let donations = runners.reduce((accumulator,item) =>
